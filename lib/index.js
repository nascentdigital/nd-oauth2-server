...
class OAuth2Server {
    ...
    authenticateSocket(scopes) {
        ...
        return function (socket, next) {
            ...
            // try to get authorization from query parameters
            let authorization;
            ...
            if (urlComponents.length === 2) {
                ...
                const queryParams = querystring.parse(urlComponents[1]);
                const accessToken = queryParams['token'];

                // create bearer from token if found
                if (accessToken) {
                    authorization = 'Bearer ' + accessToken;
                }
            }

            // try to get authorization from cookies if not found yet
            if (!authorization) {
            ...
            // fail if authorization still isn't set
            if (!authorization)
            {
